...
std::string ext="/media/zzy/T7/ship_c_data/lidar-camera/extrinsic-left.txt";

double fx=1.121996820617591e+03;
double fy=1.121713320959861e+03;
double cx=1.015384051830748e+03;
double cy=4.970475695528236e+02;
...
public:
        ...
        : it_(nh)
    {
        ...
        // 加载图像和点云文件夹
        loadFiles("/media/zzy/T7/ship_c_data/lidar-camera/camera/test", "/media/zzy/T7/ship_c_data/lidar-camera/2024-05-15-16-58-27_filter/test");
...
private:
    ...
    void processImageAndPublish(const std::string& image_file, const std::string& pcd_file)
    {
       ...
       // cv::imwrite(image_file,projected_image);
...
{
    // 读取图像
    cv::Mat input_image = cv::imread("/media/zzy/T7/ship_c_data/lidar-camera/data-new/1.jpg");

    // 读取点云
    pcl::PointCloud<pcl::PointXYZI>::Ptr lidar_cloud(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::io::loadPCDFile<pcl::PointXYZI>("/media/zzy/T7/ship_c_data/lidar-camera/data-new/1.pcd", *lidar_cloud);
    cv::Mat projected_image = Proj2Img(input_image, lidar_cloud, 1, ext, fx, fy, cx, cy, k1, k2, p1, p2, k3, false);
    cv::imwrite("/media/zzy/T7/ship_c_data/lidar-camera/camera/test/2.0.jpg",projected_image);
