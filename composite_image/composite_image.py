...
class CompositeImage:
    ...
    def __init__(self, mode, video_path, start_t = 0, end_t = 999, skip_frame = 1, alpha_weight = 1.0):
    ...
    def max_variation_update(self, image, alpha):
        ...
        self.diff_img = self.diff_img * diff_mask + delta_img * delta_mask * alpha
        self. diff_norm = np.linalg.norm(self.diff_img, axis=2)
        self.abs_diff_norm = np.abs(self.diff_norm)

    def min_value_update(self, image, alpha):
        image_norm = np.linalg.norm(image, axis=2)
        ...
        self.diff_img = (new_min_img - self.ave_img) * alpha

    import numpy as np

    def max_value_update(self, image, alpha):
        image_norm = np.linalg.norm(image, axis=2)
        cur_min_image = self.diff_img + self.ave_img
        cur_min_image_norm = np.linalg.norm(cur_min_image, axis=2)
        delta_mask = cur_min_image_norm < image_norm
    ...
    def extract_frames(self):
        ...
        while video.isOpened() and frame_count <= (end_frame - start_frame):
            ...
            if ret:
                frame_count += 1
                if (frame_count != 1 and frame_count % self.skip_frame != 0):
                    continue
    ...
    def merge_images(self):
        ...
        for image_file in image_files:
            ...
            # 计算透明度权重：帧数越靠前越透明，越靠后越不透明
            progress = cnt / img_num  # 从 1/img_num 到 1.0
            alpha = 1.0 - self.alpha_weight * (1.0 - progress)  # 应用权重
            image = image_file.astype(np.float32)
            if(self.mode == CompositeMode.MAX_VARIATION):
                self.max_variation_update(image, alpha)
            elif(self.mode == CompositeMode.MIN_VALUE):
                self.min_value_update(image, alpha)
            elif(self.mode == CompositeMode.MAX_VALUE):
                self.max_value_update(image, alpha)
...
parser.add_argument('--alpha_weight', default=1.0, type=float, help='progressive transparency weight (0.0-1.0, default: 1.0).')
...
print(" -- Load Param: alpha_weight", alpha_weight)
...
merger = CompositeImage(mode, path, start_t, end_t, skip_frame, alpha_weight)
